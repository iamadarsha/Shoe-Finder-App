...
function ShoeCard({ shoe, rank }: { shoe: ShoeRecommendation; rank: number }) {
  ...
  return (
    <div
      className={`relative rounded-2xl p-6 transition-all duration-500 hover:scale-[1.01] ${
    ...
    >
      ...
      {/* Category */}
      <div
        className="inline-block px-2.5 py-1 rounded-md text-[11px] font-medium mb-4"
        style={{
          background: '#1A1A2A',
          color: '#8888A0',
      ...
      <div className="space-y-2 mb-5">
        {shoe.foam && (
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold tracking-wider uppercase w-12" style={{ color: '#55556A' }}>
              FOAM
            </span>
            <span className="text-xs" style={{ color: '#A78BFA', fontFamily: "'Figtree', sans-serif" }}>
              {shoe.foam}
            </span>
          </div>
        )}
        {shoe.plate && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs" style={{ color: '#FF9F43', fontFamily: "'Figtree', sans-serif" }}>
              {shoe.plate}
            </span>
          </div>
        )}
      ...
      <div className="flex gap-2">
        {shoe.buyLinks.amazon && (
          <a
            ...
            className="flex-1 py-2.5 rounded-lg text-center text-sm font-medium transition-all duration-200 hover:bg-white/10"
            style={{
        ...
        {shoe.buyLinks.official && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex-1 py-2.5 rounded-lg text-center text-sm font-medium transition-all duration-200 hover:opacity-90"
            style={{
              background: '#7C5CFC18',
  ...
  return (
    <div className="rounded-2xl p-6 animate-pulse" style={{ background: '#111118', border: '1px solid #1A1A2A' }}>
      <div className="flex justify-between mb-4">
        ...
        <div className="h-8 w-16 rounded-full" style={{ background: '#1A1A2A' }} />
      </div>
      <div className="h-4 w-24 rounded mb-4" style={{ background: '#1A1A2A' }} />
      <div className="space-y-2 mb-5">
        <div className="h-3 w-full rounded" style={{ background: '#1A1A2A' }} />
        <div className="h-3 w-3/4 rounded" style={{ background: '#1A1A2A' }} />
      </div>
      <div className="h-20 rounded-xl mb-5" style={{ background: '#0A0A0F' }} />
      ...
      <div className="flex items-center justify-between px-6 md:px-12 py-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: '#55556A', fontFamily: "'Figtree', sans-serif" }}>
            AI-curated from 233 shoes across 15 brands
          </p>
        </div>
        <button
          onClick={onRetake}
